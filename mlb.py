...
Free, no key. Quick commands read the schedule (with hydrated linescore).
Detailed commands (LINE / SCORING / PITCH / FULL) read the live game feed,
which contains the per-inning linescore, scoring plays, and the full
boxscore (pitching lines) - for the game in progress, or the most recently
completed game if none is live. All times are Eastern.
"""
import logging
...
# Common play events -> short tokens for the scoring summary
EVENT_SHORT = {
...
# --------------------------------------------------------------------------
# Quick commands (schedule only) - SCORE / NEXT / LAST
# --------------------------------------------------------------------------
...
def _fmt_upcoming(game, prefix="Next"):
    side = _jays_side_schedule(game)
    ...
    return (f"{prefix}: TOR {vs} {_short(opp)}, "
            f"{util.fmt_day_et(dt)} {util.fmt_time_et(dt)}{loc}{note}")


def jays_now():
    """SCORE - live line if playing, else today's result, else next game."""
...
def jays_next():
    """NEXT - next scheduled game: opponent, location, start time (ET)."""
    games = _schedule_games()
    now = util.now_et()
    for g in games:
        ...
        if abstract != "Final" and dt and dt > now:
            return _fmt_upcoming(g, prefix="Next")
...
# --------------------------------------------------------------------------
# Detailed commands (live feed) - LINE / SCORING / PITCH / FULL
# --------------------------------------------------------------------------
def _target_game():
    """Game in progress, else the most recently completed one."""
    games = _schedule_games()
...
def _score_line(feed):
    ...
    return (f"{away.get('abbreviation', 'AWY')} {a_r} @ "
            f"{home.get('abbreviation', 'HOM')} {h_r} - {_state_str(feed)}")


def line(feed):
    """Inning-by-inning runs plus R/H/E totals."""
    ls = feed.get("liveData", {}).get("linescore", {})
    away, home, _ = _feed_teams(feed)
...
def scoring(feed):
    """Every scoring play: inning, batter, result, RBIs, running score."""
...
def _pitcher_inning_ranges(feed):
    """Map each pitcher id -> the inning range they appeared in, from play-by-play."""
    ranges = {}
...
def pitching(feed):
    """Jays pitchers in order: innings pitched, earned runs, H, K, innings used."""
    ...
    for pid in side.get("pitchers", []):
        ...
        out.append(f"{_lastname(name)} {ip}IP {er}ER {hits}H {k}K{rng_s}")
    if not out:
...
def full(feed):
    """Everything: score, inning-by-inning line, scoring plays, pitching."""
    return "\n\n".join([
        _score_line(feed),
        line(feed),
        scoring(feed),
...
# ---- command entry points that resolve the target game and fetch the feed ----
